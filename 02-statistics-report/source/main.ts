...
const computeDegreeDistributions = (graph: Graphology.default) => {
    ...
    const weightedDegrees = new Map<number, number>();
    const weightedInDegrees = new Map<number, number>();
    const weightedOutDegrees = new Map<number, number>();
...
const computeStatistics = () => {
    ...
    const outputFileBasenameArgument = argument.outputFileBasename as string;
